...
public class edgeHighlights_color implements PlugInFilter
{
	...
	public void run(ImageProcessor ip)
	{
		...
		for(int u = 0;u<w;u++)
		{
			for(int v = 0;v<h;v++)
			{
				...
				double p = 2.5*Grad_ip.getPixel(u, v);
				
				rgb_array[0]= (rgb_array[0]+(int)p)/2;
				if(rgb_array[0]>255) rgb_array[0]=255;
				
				rgb_array[1]= (rgb_array[1]+(int)p)/2;
				if(rgb_array[1]>255) rgb_array[1]=255;
				
				rgb_array[2]= (rgb_array[2]+(int)p)/2;
				if(rgb_array[2]>255) rgb_array[2]=255;
